...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Class <? extends Message> , Callback> mapMessageToAction;
    ...
    public MicroService(String name) {
        this.name = name;
        mapMessageToAction = new ConcurrentHashMap<>();
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        mapMessageToAction.put(type,callback);
    ...
    public final void run() {
        ...
        while (!terminated) {
            try{
                ...
                Callback callbackFunction = mapMessageToAction.getOrDefault(m.getClass(), null);
                if(callbackFunction == null){
                    System.out.println(this.getName()+" has no actions available");
                }
                else{
